...
# Handle deletion if DeleteBefore is specified
if ($DeleteBefore) {
    $filesToDelete = $files | Where-Object { 
        if ($TimeType -eq 'Creation') {
            $_.CreationTime -lt $DeleteBefore
        } else {
    ...
    if ($filesToDelete) {
        Write-Host "`nFiles to delete (older than $DeleteBefore):" -ForegroundColor Red
        foreach ($file in $filesToDelete) {
            $timestamp = if ($TimeType -eq 'Creation') { $file.CreationTime } else { $file.LastWriteTime }
            Write-Host "  $($file.FullName) - $timestamp"
        }
        
        if (-not $DryRun) {
            $confirmation = Read-Host "`nAre you sure you want to delete these files? (yes/no)"
            ...
            Write-Host "`nDry run - no files will be deleted" -ForegroundColor Yellow
        }
    } else {
        Write-Host "No files found older than $DeleteBefore"
    }
}
